...
E\left(h_n\left(X, 1\right) h_m\left(X, 1\right)\right)  =
\left\{
\begin{array}{lr}
n! & \text{za } m = n \\
0 & \text{za } m \neq n 
...
\noindent (ii) Naj bo $a \in \mathbb{C}$ in $c \in \mathbb{R}$. Izrazi $E\left( e^{a\left(X+cY\right)}|X\right)$ z $e_a$, $X$ in $c$ ter $E\left(\left(X+cY\right)^{k}|X\right)$ za $k \in \mathbb{N}_0$ z $h_k$, $X$, in $c$.
\\[10px]
 
\noindent Najprej upoštevamo lastnosti pogojnega matematičnega upanja in uporabimo neodvisnost. Tako dobimo:
\begin{equation*}
\begin{aligned}
E\left( e^{a\left(X+cY\right)}|X\right)=E\left( e^{aX}e^{acY}|X\right)=e^{aX}E\left(e^{acY}\right).
\end{aligned}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
E\left( e^{a\left(X+cY\right)}|X\right)=E\left(\sum_{n=0}^{\infty}\frac{a^n\left(X+cY\right)^n}{n!}|X\right)=\sum_{n=0}^{\infty}\frac{a^n}{n!}E((X+cY)^n|X). 
\end{aligned}
\end{equation*}
